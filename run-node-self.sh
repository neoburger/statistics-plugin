...
mkdir -p data
mkfifo .tmp
cat .tmp | (dotnet ../.node/neo-cli/bin/Release/net6.0/neo-cli.dll >log 2>&1 && echo > .tmp)
...
az storage blob upload-batch --max-connections 200 --overwrite true --destination data --source .tmpdir/data --account-name neoburger --account-key ${TODO} // TODO
az storage blob upload --overwrite true --container-name data --file .tmpdir/index --account-name neoburger --account-key ${TODO} // TODO
